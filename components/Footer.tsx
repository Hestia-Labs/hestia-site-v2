    ...
    <footer className="bg-black text-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-center">
            ...
            <TransitionLink href="/cleaning-contact">
              <button className="bg-black hover:bg-gray-900 text-white font-bold py-4 px-8 rounded text-lg transition-colors">
                GET FREE QUOTE
              </button>
            </TransitionLink>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-16 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
          ...
          {/* Quick Links */}
          <div className="md:col-span-1">
            <h3 className="font-bellefair text-xl mb-6 text-yellow-500">QUICK LINKS</h3>
            <ul className="space-y-3">
              {footerContent.quickLinks.map((link) => (
                <li key={link.title}>
                  <TransitionLink
                    href={link.href}
                    className="text-gray-400 hover:text-yellow-500 transition-colors flex items-center group"
                  >
          ...
          {/* Service Areas */}
          <div className="md:col-span-1">
            <h3 className="font-bellefair text-xl mb-6 text-yellow-500">SERVICE AREAS</h3>
            <ul className="space-y-3">
              {footerContent.contact.address.map((area, index) => (
                <li key={index} className="flex items-start text-gray-400">
                  <MapPin className="w-5 h-5 text-yellow-500 mr-3 flex-shrink-0 mt-0.5" />
                  <span>{area}</span>
                </li>
              ))}
            </ul>
          ...
          {/* Certifications */}
          <div className="md:col-span-1">
            <h3 className="font-bellefair text-xl mb-6 text-yellow-500">CERTIFICATIONS</h3>
            <div className="space-y-4">
              {footerContent.certifications.map((cert, index) => {
                const Icon = cert.icon;
                return (
                  <div key={index} className="flex items-start">
                    <Icon className="w-6 h-6 text-yellow-500 mr-3 flex-shrink-0 mt-0.5" />
                    <div>
        ...
        <div className="mb-12">
          <div className="bg-gray-900 rounded-lg p-8">
            <div className="max-w-2xl mx-auto text-center">
              <h3 className="font-bellefair text-2xl mb-4 text-yellow-500">
                STAY UPDATED
              </h3>
              <p className="text-gray-400 mb-6">
              ...
              <form className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto" onSubmit={handleSubmit}>
                <input
                  ...
                  required
                  className="flex-1 px-4 py-3 bg-black border border-gray-700 rounded focus:outline-none focus:border-yellow-500 text-white placeholder-gray-500"
                />
                <button
                  type="submit"
              ...
              {formStatus.success && (
                <p className="text-yellow-500 mt-3">Thank you for subscribing!</p>
              )}
              {formStatus.error && (
